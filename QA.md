...
| Folder stacks | Add a folder from the picker or Finder drop, click it, right-click it, change Sort By, Display as, and View content as. Use Primary Dock match on a Dock with Applications/Downloads folders. | Folder click opens a stack panel anchored to the folder icon; clicking the same icon closes it; Open opens Finder; Sort By/Display as/View content as affect the item; Remove from Docking never deletes the folder; Primary Dock match imports `persistent-others`. | Partially covered 2026-06-29 by validation: Apple Dock mirroring imports `persistent-others` directory tiles with `displayas`/`showas`/`arrangement`; app catalog accepts folders and rejects plain files; folder stack sorting and Automatic/Fan/Grid/List presentation resolve correctly; stack hit testing keeps inside clicks and same-folder toggle clicks from being treated as outside dismissals while preserving normal outside-click dismissal. Live Finder drag/drop and visual stack-panel inspection are still not yet manually verified. |
