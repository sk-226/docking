...
| Appearance and magnification | Adjust Dock size, widget size, and magnification in Appearance. Test minimum/maximum sizes and bottom/left/right placement. Open a widget and the add picker. | Icons and the panel share dimensions; widgets do not increase Dock thickness; nearby icons expand without overlapping; the small add button stays clickable. | Partially verified 2026-09-19 in Tart (macOS 26.6.2): size sliders, static maximum magnification, pointer exit, bottom/left/right layout, Weather open/close, and the add picker. A 10-second deterministic motion replay produced 619 display callbacks over 10.30 seconds including settling, median 16.69 ms and maximum 17.81 ms, with one fixed window frame. Validation covers reversal, refresh-rate-independent timing, crowded layouts, every edge, imports, and persistence. Exact native motion parity remains open: the target origin still oscillates between icon centers. See MAGNIFICATION_HANDOFF.md. Multi-display and accessibility preference changes were not manually exercised. |
