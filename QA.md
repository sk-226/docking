...
| Appearance and magnification | Adjust Dock size, widget size, and magnification in Appearance. Test minimum/maximum sizes and bottom/left/right placement. Open a widget and the add picker. | Icons and the panel share dimensions; widgets do not increase Dock thickness; nearby icons expand without overlapping; the small add button stays clickable. | Partially verified 2026-09-19 in Tart (macOS 26.6.2, Swift 6.2.3): all 10 magnification tests, the 60-check validation executable, app build/verify, and launch smoke passed. Live checks covered bottom/left/right placement at maximum magnification, exit settlement, and activation through an enlarged icon. A deterministic motion replay held the interior target origin at 92 pt and the window frame constant; active display intervals had median 16.67 ms and maximum 18.06 ms. Earlier checks covered size sliders, Weather open/close, and the add picker. Exact native motion parity, multi-display, full-screen, auto-hide interaction, and accessibility preference changes still need manual verification. See MAGNIFICATION_HANDOFF.md. |
